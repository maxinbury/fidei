...
router.post('/add', isLoggedIn,isLevel2, async (req, res)=>{
    ...
    if  (row.length > 0){req.flash('message','Error dni ya existe')
...
router.post('/listadni',isLoggedIn,isLevel2, async(req, res, next) =>{
    ...
    }else {req.flash('message','Error, Dni no encontrado ')
...
router.post('/listapp',isLoggedIn,isLevel2, async(req, res, next) =>{
    ...
    }else {req.flash('message','Error, Apellido no encontrado ')
           res.redirect('clientes')}
})
